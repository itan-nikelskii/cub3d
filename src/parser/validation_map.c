...
/*   Updated: 2026/01/12 13:13:23 by mgroos           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_chars(t_map *map, t_data *data)
{
	int	i;
	int	j;
...
static void	check_walkable_cell(t_map *map, int y, int x, t_data *data)
{
	...
	while (i < map->height)
	{
		...
		while (j < map->width)
		{
			if (map->grid[i][j] == '0' || is_player(map->grid[i][j]))
				check_walkable_cell(map, i, j, data);
			j++;
		}
		i++;
	}
